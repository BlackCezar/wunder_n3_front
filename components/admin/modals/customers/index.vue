<template>
    <form ref="editClientForm" @submit="saveCustomer">
        <b-tabs nav-class="tabs-header" content-class="p-4 w-100" fill v-model="activeTab">
            <b-tab title-item-class="top-up-tab" active-tab-class="active" :title="$t('EditClient.RegData')">
                <AdminMainDataTab />
            </b-tab>
            <b-tab :title="$t('EditClient.CompanyData')" title-item-class="top-up-tab" active-tab-class="active">
                <AdminCompanyDataTab />
            </b-tab>
            <b-tab :title="$t('EditClient.ResponsiblePersonData')" title-item-class="top-up-tab" active-tab-class="active">
                <AdminLiderInfoTab />
            </b-tab>
            <b-tab :title="$t('EditClient.BankData')" title-item-class="top-up-tab" active-tab-class="active">
                <AdminBankInfoTab />
            </b-tab>
            <b-tab :title="$t('EditClient.AccountData')" title-item-class="top-up-tab" active-tab-class="active">
        ...
        <b-button style="width: 100%; height: 59px" :disabled="!meta.valid || isLoading" variant="danger" type="submit">
            <template v-if="isLoading">{{ $t("AccountManagement.Save") }}
...
const schema = Yup.object().shape({
    ...
    companyPhoneNumber: Yup.string().optional().nullable().default(null),
    dateOfBirth: Yup.string().optional().nullable().default(null),
...
const saveCustomer = handleSubmit(async (values) => {
    ...
    if (values) {
        ...
        useEvent("modal-close:edit-client", values as ICustomer);
        resetForm();
    }
});
</script>
